...
def main(config, pruning_settings, log_filename):
    ...
    server_model.fit(pruning=config.environment.pruneconv, log_file=log_filename, pruning_settings=pruning_settings)
...
if __name__ == '__main__':
    ...
    config.environment.paoding = 0
    config.environment.pruneconv = 1
    ...
    if TUNING_PRUNING:
        for prune_freq in [1]:
            ...
            for target in [0.01, 0.02, 0.05, 0.1, 0.25, 0.5]:
                ...
                curr_exp_settings.append('prune')
                if exp_idx >= RESUME:
    ...
    if BASE_SETTINGS:
        ...
        for prune_option in [1]:
            config.environment.pruneconv= prune_option
            curr_exp_settings = []
            curr_exp_settings.append(config.dataset.dataset)
